        ...
        if (result.data.total != "0") { 
            ...
            for (const dep of totalDeps) {
                ...
                for (const project of dep.projects) {
                    ...
                    writeToCSV(`${url.orgSlug},${url.orgId},${dep.id?.replace(',',';')},${dep.name},${dep.version?.replace(',',';')},${dep.latestVersion},${dep.latestVersionPublishedDate},${dep.firstPublishedDate},${dep.isDeprecated},${dep.licenses},${project.name},${project.id},${projectUrl}`)
                }
...
async function app() {
    ...
    writeToCSV(`org-slug,org-id,dep-id,dep-name,dep-version,latest-version,latest-version-published-date,first-published-date,is-deprecated,licens(es),project-name,project-id,project-url`)
    let queue = [];
